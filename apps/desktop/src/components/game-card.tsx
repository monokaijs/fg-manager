...
// Thin wrapper that only subscribes to download store when needed.
// This avoids creating 100 Zustand subscriptions for the catalog view
// where showStatus is false and no game has an active download.
function useGameTask(slug: string, enabled: boolean) {
  return useDownloadStore(state => {
    if (!enabled) return undefined;
    return state.tasks.find((t) => t.gameSlug === slug);
  });
}

export const GameCard = React.memo(function GameCard({ game, viewMode = "grid", showStatus = false }: GameCardProps) {
  const task = useGameTask(game.slug, showStatus);
      ...
      <Link to={`/games/view/${game.slug}`} className="flex items-center space-x-4 p-3 rounded-lg border border-border/50 bg-muted/10 hover:bg-muted/30 hover:border-border transition-colors cursor-pointer group">
        ...
        {task && (
          <div className="w-24 shrink-0 mx-2 h-1.5 bg-muted/50 rounded-full overflow-hidden self-center border border-border/50">
             <div className="h-full bg-primary" style={{ width: `${(task.progress || 0) * 100}%` }} />
          </div>
        )}
  ...
  // - contain:content for render isolation
  // - Hardware scrolling matrix overlaps: Removed hover:-translate to prevent 180Hz hover collisions
  return (
    <Link
      to={`/games/view/${game.slug}`}
      className="group relative aspect-[3/4] rounded-xl bg-muted/30 border border-border/50 overflow-hidden transition-opacity cursor-pointer flex flex-col"
      style={{ contain: 'content', transform: 'translateZ(0)' }}
    >
      {game.postImage ? (
        <img decoding="async" src={game.postImage} alt={decodedTitle} className="absolute inset-0 w-full h-full object-cover" />
      ) : (
        ...
        <div className="absolute top-3 right-3 bg-background/90 rounded-full p-0.5 z-30 flex items-center justify-center border border-primary/20 shadow-sm">
          <div className="relative w-8 h-8 flex items-center justify-center">
            <svg className="w-8 h-8 -rotate-90">
              ...
              <circle className="text-primary" strokeWidth="2.5" strokeLinecap="round" stroke="currentColor" fill="transparent" r="14" cx="16" cy="16" strokeDasharray={`${task.progress * 87.96} 87.96`} />
            </svg>
          </div>
        </div>
      )}
      ...
      <div className="absolute bottom-4 left-4 right-4 z-20">
        ...
        <p className="text-xs text-muted-foreground bg-background border border-border/40 w-max px-1.5 py-0.5 rounded capitalize transform-gpu">
